document.addEventListener('DOMContentLoaded', () => {
    ...
    navButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            button.classList.add('active');document.addEventListener('DOMContentLoaded', () => {
    // DOM
    const navButtons = document.querySelectorAll('.nav-btn');
    const tabContents = document.querySelectorAll('.tab-content');
    const sideMenu = document.getElementById('side-menu');
    ...
    // Normaliza un SKU para poder compararlo/buscarlo sin importar mayúsculas,
    // espacios extra o diferencias de acentos entre lo escaneado y el catálogo.
            ...
            Papa.parse(csvData, {
                ...
                complete: function (results) {
                    ...
                    results.data.forEach(row => {
                        ...
                        if (sku) newCatalog[normalizeSku(sku)] = { // Clave normalizada para que el escaneo siempre matchee
                            skuOriginal: String(sku).trim(),
                            descripcion: descripcion || '',
                            ubicacion: ubicacion // Se guarda la ubicación limpia
                        };
                    });
                    ...
                    await zxingCodeReader.decodeFromVideoDevice(deviceId || null, videoElem, (result, err) => {
                        if (result && result.text) {
                            if (currentScanInput) {
                                ...
                                currentScanInput.dispatchEvent(new Event('input'));
                                currentScanInput.dispatchEvent(new CustomEvent('scan-complete', { detail: { code: result.text } }));
                                try {
                                    zxingCodeReader.reset();
                                } catch (e) {}
                                stopScanner();
    ...
    function renderData() {
        ...
        // Columnas para renderizar la tabla de Guardado
        const guardadoCols = [{
            key: 'fecha',
            title: 'Fecha'
        }, {
    ...
    // Guarda una entrada de Guardado (sku + ubicación colocada), igual que las otras ventanas
    async function saveGuardadoEntry(sku, descripcion, ubicacion) {
        const fecha = new Date().toLocaleString();
        if (firebaseEnabled) {
            await guardadoRef.push({
                fecha,
                sku,
                descripcion: descripcion || '',
                ubicacion
    ...
    // Muestra el "cartel" con la descripción y ubicación del SKU escaneado/ingresado.
    // Si el catálogo tiene ubicación, se guarda automáticamente y el cartel es solo informativo.
    // Si no tiene ubicación, permite escribirla y guardarla manualmente.
    function showGuardadoCard(skuRaw) {
        const sku = String(skuRaw || '').trim();
        const productInfo = getCatalogEntry(sku);
        const descripcion = productInfo ? productInfo.descripcion : '';
        const ubicacionCatalogo = productInfo && productInfo.ubicacion ? String(productInfo.ubicacion).trim() : '';
        ...
        if (ubicacionCatalogo) {
            ...
            locP.querySelector('strong').textContent = ubicacionCatalogo;
        ...
        if (!ubicacionCatalogo) {
            ...
            saveBtn.addEventListener('click', async () => {
                ...
                await saveGuardadoEntry(sku, descripcion, finalLocation);
                document.body.removeChild(overlay);
                guardadoForm.reset();
            });
        ...
        // Si ya tenía ubicación en el catálogo, se guarda automáticamente al mostrar el cartel
        if (ubicacionCatalogo) {
            saveGuardadoEntry(sku, descripcion, ubicacionCatalogo);
        }
    }

    // Escaneo: al terminar de escanear un SKU en la pestaña Guardado, se abre el cartel automáticamente
    guardadoSkuInput.addEventListener('scan-complete', (e) => {
        const sku = (e.detail && e.detail.code ? e.detail.code : guardadoSkuInput.value).trim();
        if (sku) showGuardadoCard(sku);
    });

    // Ingreso manual del SKU (sin cámara): buscar/registrar mediante el formulario
    guardadoForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const sku = guardadoSkuInput.value.trim();
        if (!sku) return;
        showGuardadoCard(sku);
    });
