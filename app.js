...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function aggregateAndExport(dataArray, filenamePrefix) {
    ...
    const out = Object.values(agg).map(it => ({ SKU: it.SKU, CANTIDAD: it.CANTIDAD, TXT: `${it.SKU},${it.CANTIDAD}`, FECHA: it.FECHA, UBICACIONES: it.UBICACIONES.join(' ; ') }));
    const cols = [{key:'SKU',title:'SKU'},{key:'CANTIDAD',title:'CANTIDAD'},{key:'TXT',title:'TXT'},{key:'FECHA',title:'FECHA'},{key:'UBICACIONES',title:'Ubicaciones'}];
  ...
  document.getElementById('export-movimientos-btn').addEventListener('click', () => {
    const today = new Date().toISOString().slice(0,10);
    const cols = [{key:'fecha',title:'FECHA'},{key:'origen',title:'ORIGEN'},{key:'destino',title:'DESTINO'},{key:'sku',title:'SKU'},{key:'cantidad',title:'CANTIDAD'}];
    exportToCsv(`Movimientos_${today}.csv`, movimientosData, cols);
  });
